...
<body>
	...
	<form
		action="<%=request.getContextPath()%>/quickloud-master/cart/orderAllAction.jsp"
		...
		<div class="basketdiv" id="basket">
			...
			for (Map<String, Object> c : cartList) {
				...
				totalcount += (int) c.get("cartQuantity");
				...
				<div class="subdiv">
					<div class="check">
						<input type="checkbox" name="buy" value="260" checked="checked"
							onclick="javascript:basket.checkItem();" disabled>&nbsp;
					</div>
					<div class="img">
						<img
							src="<%=request.getContextPath()%>/upload/<%=c.get("filename")%>"
							width="60">
					</div>
					<div class="pname">
						<span><%=c.get("goodsName")%></span>
					</div>
				</div>
				<div class="subdiv">
					<div class="basketprice">
						<input type="hidden" name="p_price" id="p_price1" class="p_price"
							value="<%=c.get("goodsPrice")%>"><%=c.get("goodsPrice")%></div>
					<div class="num">
						<div class="updown">
							<input type="text" name="p_num<%=itemCount%>"
								id="p_num<%=itemCount%> %>" size="2" maxlength="4" class="p_num"
								value="<%=c.get("cartQuantity")%>"
								onkeyup="javascript:basket.changePNum(<%=itemCount%>);">
							<span onclick="javascript:basket.changePNum(<%=itemCount%>);"><i
								class="fas fa-arrow-alt-circle-up up"></i></span> <span
								onclick="javascript:basket.changePNum(<%=itemCount%>);"><i
								class="fas fa-arrow-alt-circle-down down"></i></span>
						</div>
					</div>
					<div class="sum"><%=totalPrice%>원
					</div>
				</div>
				<div class="subdiv">
					<div class="basketcmd">
						<a
							href="<%=request.getContextPath()%>/quickloud-master/cart/removeCart.jsp?goodsNo=<%=c.get("goodsNo")%>"
		...
		<div class="right-align basketrowcmd">
			<a
				href="<%=request.getContextPath()%>/quickloud-master/cart/removeAllCart.jsp"
				class="btn btn-primary grd1 effect-1 btn-radius btn-brd"
		...
		<div class="row">
			...
			<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
				...
				<input type="text" name="customerDetailAddress"
