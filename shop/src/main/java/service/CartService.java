...
public class CartService {
	...
	public int removeAllCart(Cart cart) {
		...
		try {
			...
			row = cartDao.deleteAllCart(conn, cart);
	...
	public boolean orderCartOne(Map<String, Object> map,Orders orders) {
		...
		try {
			...
			if(ordersDao.ordersInsert(conn, orders) != 1) {
				throw new Exception(); // 오류는 발생하지 않았지만 삭제는 안 된 경우
			}
